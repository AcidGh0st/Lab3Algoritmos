...
public class DivideConquerController {
    ...
    public void createOnAction(ActionEvent actionEvent) {
        if (this.isValid()) {
            int size = Integer.valueOf(this.nTextField.getText());
            Random random = new Random();
            this.vector = new Vector(size);
            for (int i = 0; i < size; i++) {
                this.vector.add(random.nextInt(size)); // Modifica 100 según el rango de números aleatorios que necesites
            }
            this.textArea.setText("The vector has been created with " + size + " random elements.");
            this.buttonsPane.setDisable(false);
        } else {
            this.alert.setAlertType(Alert.AlertType.ERROR);
            this.alert.setContentText("Please complete the info and try again...");
